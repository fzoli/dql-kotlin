...
class DqlMethodInvokerDoubleStrategy(private val expr: DoubleField) : DqlMethodInvokerStrategy {
    ...
    override fun resolveNumberExpression(field: String, method: String, number: DqlNumber): Criteria {
        return when (method) {
            DqlMethods.EQ -> expr.eq(number.toDouble())
            DqlMethods.GT -> expr.gt(number.toDouble())
            DqlMethods.GOE -> expr.goe(number.toDouble())
            DqlMethods.LOE -> expr.loe(number.toDouble())
            DqlMethods.LT -> expr.lt(number.toDouble())
            else -> throw unknownMethodException(field, method, expr)
        }
    }
    ...
    override fun resolveNumberListExpression(field: String, method: String, numberList: List<DqlNumber>): Criteria {
        rejectDuplicates(field, numberList)
        return when (method) {
            DqlMethods.MEMBER_OF -> expr.memberOf(numberList.map { it.toDouble() })
            else -> throw unknownMethodException(field, method, expr)
        }
    }
