...
class DqlMethodInvokerStringStrategy(private val expr: StringField) : DqlMethodInvokerStrategy {
    ...
    override fun resolveUnaryExpression(
        ...
        return when (method) {
            ...
            DqlMethods.IS_EMPTY -> expr.isEmpty()
            DqlMethods.IS_NOT_EMPTY -> expr.isNotEmpty()
    ...
    override fun resolveStringExpression(
        ...
        return when (method) {
            DqlMethods.EQ -> expr.eq(string)
            DqlMethods.EQUALS_IGNORE_CASE -> expr.equalsIgnoreCase(string)
            DqlMethods.CONTAINS -> expr.contains(string)
            DqlMethods.CONTAINS_IGNORE_CASE -> expr.containsIgnoreCase(string)
            DqlMethods.STARTS_WITH -> expr.startsWith(string)
            DqlMethods.STARTS_WITH_IGNORE_CASE -> expr.startsWithIgnoreCase(string)
            else -> throw unknownMethodException(field, method, expr)
        }
    }
    ...
    override fun resolveStringListExpression(
        ...
        return when (method) {
            DqlMethods.MEMBER_OF -> expr.memberOf(stringList)
            else -> throw unknownMethodException(field, method, expr)
        }
    }
