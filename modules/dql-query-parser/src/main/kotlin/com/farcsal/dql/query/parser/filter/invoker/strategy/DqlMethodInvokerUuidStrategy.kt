...
class DqlMethodInvokerUuidStrategy(private val expr: UuidField) : DqlMethodInvokerStrategy {
    ...
    override fun resolveStringExpression(
        ...
        return when (method) {
            DqlMethods.EQ -> expr.eq(UUID.fromString(string))
            else -> throw unknownMethodException(field, method, expr)
        }
    }
    ...
    override fun resolveStringListExpression(
        ...
        return when (method) {
            DqlMethods.MEMBER_OF -> expr.memberOf(stringList.map { UUID.fromString(it) })
            else -> throw unknownMethodException(field, method, expr)
        }
    }
